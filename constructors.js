...
// Use these parameters to create a new object and add it to the savedPosts array. Make sure to name the properties the same as
// described previously (id, title, rating).
...
//       return this.savedPosts.splice(i, 1);
//     }
//   }
// };
...
User.prototype.removeSavedPost = function(postId) {
  let index = this.savedPosts.findIndex(el => postId === el.id)
...
// that will take in two number parameters. The first will be an id (a number) and the second will be the new rating (a number). 
...
User.prototype.changePostRating = function(postId, rating) {
  let index = this.savedPosts.findIndex(el => postId === el.id)
